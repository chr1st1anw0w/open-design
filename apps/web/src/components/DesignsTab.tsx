      ...
      {filtered.length === 0 ? (
        projects.length === 0 ? (
          <div className="design-grid">
            <GardenDesignCard />
            <GptImageToolCard
              title="GPT-Image 2 Prompt Studio"
              desc="結構化提示詞分類、模板填表與歸檔。"
              onClick={() => navigate({ kind: 'tool', toolId: 'gpt-image2', page: 'prompt-studio' })}
            />
            <GptImageToolCard
              title="GPT-Image 2 Workbench"
              desc="快速選模板、調參數、即時渲染提示詞。"
              onClick={() => navigate({ kind: 'tool', toolId: 'gpt-image2', page: 'workbench' })}
            />
          </div>
        ) : (
...
function GardenDesignCard() {
  return (
    <div
      className="design-card garden-design-card"
      role="button"
      tabIndex={0}
      onClick={() => navigate({ kind: 'tool', toolId: 'web-design' })}
      onKeyDown={(e) => {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault();
          navigate({ kind: 'tool', toolId: 'web-design' });
        }
      }}
    >
      <div className="design-card-thumb garden-design-card-thumb" aria-hidden>
        <Icon name="grid" size={26} />
      </div>
      <div className="design-card-meta-block">
        <div className="design-card-name">Web Design Skill 影片範本</div>
        <div className="design-card-meta">
          Claude Design · 範本 · 可參考動畫、版式與敘事節奏
        </div>
      </div>
    </div>
  );
...
function GptImageToolCard({
  ...
  return (
    <div
      className="design-card garden-design-card gpt-image-tool-card"
      role="button"
      tabIndex={0}
      onClick={onClick}
      onKeyDown={(e) => {
    ...
    >
      <div className="design-card-thumb gpt-image-tool-thumb" aria-hidden>
        <Icon name="sparkles" size={24} />
      </div>
      <div className="design-card-meta-block">
        <div className="design-card-name">{title}</div>
        <div className="design-card-meta">{desc}</div>
      </div>
    </div>
  );
}
