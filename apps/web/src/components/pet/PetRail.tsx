...
export function PetRail({
  ...
  const pet: PetConfig = config.pet ?? {
    ...DEFAULT_PET,
  ...
  if (collapsed) {
    return (
      <aside className="pet-rail collapsed" aria-label={t("pet.railAria")}>
        ...
        >
          <span className="pet-rail-toggle-glyph" aria-hidden>
            🐾
          </span>
  ...
  return (
    <aside className="pet-rail" aria-label={t("pet.railAria")}>
      ...
      <div className="pet-rail-shortcuts">
        <div className="pet-rail-shortcuts-title">Quick Access</div>
        <button
          type="button"
          className="pet-rail-shortcut-item"
          onClick={() => {
            if (typeof window !== "undefined") {
              window.location.href = "/tools/web-design";
            }
          }}
          title="Open Web Design Tool"
        >
          <span className="pet-rail-shortcut-icon">🎨</span>
          <span className="pet-rail-shortcut-label">Web Design</span>
        </button>
        <button
          type="button"
          className="pet-rail-shortcut-item"
          onClick={() => {
            if (typeof window !== "undefined") {
              window.location.href = "/tools/gpt-image2/prompt-studio";
            }
          }}
          title="Open Prompt Studio"
        >
          <span className="pet-rail-shortcut-icon">✨</span>
          <span className="pet-rail-shortcut-label">Prompt Studio</span>
        </button>
      </div>
      ...
      <div className="pet-rail-list">
        ...
        >
          ...
          <span className="pet-rail-item-meta">
            ...
            <span className="pet-rail-item-flavor">
              {t("pet.railCustomFlavor")}
            </span>
          </span>
