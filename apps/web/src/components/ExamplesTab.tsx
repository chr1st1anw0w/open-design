...
type ExampleView = 'large' | 'medium' | 'small' | 'list';
...
export function ExamplesTab({ skills, onUsePrompt }: Props) {
  ...
  const [view, setView] = useState<ExampleView>('large');
    ...
    <div className="tab-panel examples-panel">
      <div className="examples-toolbar">
        <div className="examples-toolbar-top">
          <div className="examples-result-count">
            {filtered.length} / {skills.length}
          </div>
          <div className="examples-view-controls" role="group" aria-label="範例檢視模式">
            {(['large', 'medium', 'small', 'list'] as const).map((mode) => (
              <button
                key={mode}
                type="button"
                className={`examples-view-button ${view === mode ? 'active' : ''}`}
                aria-pressed={view === mode}
                title={viewTitle(mode)}
                onClick={() => setView(mode)}
              >
                <Icon name={mode === 'list' ? 'file' : 'grid'} size={14} />
                <span className="sr-only">{viewTitle(mode)}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className={`examples-list examples-list-${view}`}>
          {filtered.map((skill) => (
            <ExampleCard
              key={skill.id}
              skill={skill}
...
function viewTitle(view: ExampleView): string {
  if (view === 'large') return '大縮圖';
  if (view === 'medium') return '中縮圖';
  if (view === 'small') return '小縮圖';
  return '清單檢視';
}
...
function ExampleCard({
  ...
  return (
    <div
      className={`example-card example-card-${view}`}
      data-testid={`example-card-${skill.id}`}
      onMouseEnter={() => {
        setHovered(true);
        if (!skill.previewImageUrl) onLoad();
      }}
