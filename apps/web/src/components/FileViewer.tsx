...
interface ComponentDropPayload {
  path: string;
  title: string;
  kind?: string;
}
...
function EditPopover({
  ...
  return (
    <div className="comment-popover edit-popover" data-testid="edit-popover">
      <div className="comment-popover-head">
        <div>
          <strong>{target.elementId}</strong>
          <span>{target.text || target.label}</span>
        </div>
        <button type="button" className="ghost" onClick={onClose}>
          關閉
        </button>
      ...
      <textarea
        ...
        value={draft}
        placeholder="輸入要修改的文字、語氣、版面或元件用途..."
        onChange={(event) => onDraft(event.target.value)}
      />
      <div className="comment-popover-actions">
        <span className="edit-popover-hint">會交給目前選定的 agent 修改專案檔案</span>
        <button
          type="button"
          className="primary"
          disabled={!draft.trim()}
...
function TweakPanel({
  ...
  const chips = [
    '提升文字層級與可讀性，降低資訊密度',
    '讓這頁更像高品質簡報，強化主標與留白',
    '統一卡片、按鈕、標籤與 footer 對齊',
    '修正目前頁面的 overflow、間距與手機版適配',
  ];
  return (
    <div className="viewer-tweak-panel" data-testid="viewer-tweak-panel">
      <div className="viewer-tweak-head">
        <div>
          <strong>調整目前畫面</strong>
          <span>適合全頁風格、版面密度、對齊與視覺層級調校</span>
        </div>
        <button type="button" className="ghost" onClick={onClose}>
          關閉
        </button>
      </div>
      <div className="viewer-tweak-chips">
        {chips.map((chip) => (
          <button key={chip} type="button" onClick={() => onSend(chip)}>
            {chip}
          </button>
        ))}
      </div>
      <textarea
        value={draft}
        placeholder="描述想怎麼調整，例如：讓這頁更像投資人 pitch deck，減少文字、放大主張、保留四張卡片。"
        onChange={(event) => setDraft(event.target.value)}
      />
      <div className="viewer-tweak-actions">
        <span>單一元素文字請用「編輯」，保留意見請用「評論」。</span>
        <button type="button" className="primary" disabled={!draft.trim()} onClick={() => onSend(draft)}>
          送出調整
        </button>
      </div>
    </div>
...
function HtmlViewer({
  ...
  const [activeEditTarget, setActiveEditTarget] = useState<PreviewCommentSnapshot | null>(null);
  ...
  const useUrlLoadPreview = shouldUrlLoadHtmlPreview({
    ...
    commentMode: commentMode || editMode,
    forceInline,
  });
  ...
  const srcDoc = useMemo(
    () => (previewSource ? buildSrcdoc(previewSource, {
      ...
      baseHref: projectRawUrl(projectId, baseDirFor(file.name)),
	      ...
	      commentBridge: commentMode || editMode,
	    }) : ''),
	    [previewSource, effectiveDeck, projectId, file.name, previewStateKey, commentMode, editMode],
	  );
  ...
  useEffect(() => {
    ...
    if (!win) return;
    win.postMessage({ type: 'od:comment-mode', enabled: commentMode || editMode }, '*');
  }, [commentMode, editMode, srcDoc]);
  ...
  useEffect(() => {
    const interactionMode = commentMode || editMode;
    if (!interactionMode) {
      setActiveCommentTarget(null);
      setActiveEditTarget(null);
      setHoveredCommentTarget(null);
    ...
    window.addEventListener('message', onMessage);
    return () => window.removeEventListener('message', onMessage);
  }, [commentMode, editMode, file.name, previewComments]);

  function sendEditPrompt(snapshot: PreviewCommentSnapshot, instruction: string) {
    if (!instruction.trim() || !onSendPrompt) return;
    onSendPrompt([
      `Edit @${file.name} by changing the selected preview element.`,
      '',
      '<selected-element>',
      `elementId: ${snapshot.elementId}`,
      `selector: ${snapshot.selector}`,
      `label: ${snapshot.label}`,
      `currentText: ${snapshot.text || '(empty)'}`,
      `htmlHint: ${snapshot.htmlHint || '(none)'}`,
      '</selected-element>',
      '',
      `User edit request: ${instruction.trim()}`,
      '',
      'Apply the smallest safe change to the current project files. Preserve unrelated layout and content.',
    ].join('\n'));
    setActiveEditTarget(null);
    setEditDraft('');
  }

  function sendTweakPrompt(instruction: string) {
    if (!instruction.trim() || !onSendPrompt) return;
    const slideLine = slideState
      ? `Current slide/page: ${slideState.active + 1} / ${slideState.count}`
      : 'Current slide/page: visible preview';
    onSendPrompt([
      `Adjust @${file.name} based on the current preview.`,
      slideLine,
      '',
      `Tweak request: ${instruction.trim()}`,
      '',
      'Treat this as a design refinement task: improve hierarchy, spacing, typography, color, responsiveness, and deck/page fit as needed. Preserve the project direction and avoid rewriting unrelated sections.',
    ].join('\n'));
    setTweakPanelOpen(false);
  }

  function sendComponentDropPrompt(payload: ComponentDropPayload, instruction: string) {
    if (!instruction.trim() || !onSendPrompt) return;
    onSendPrompt([
      `Integrate component source @${payload.path} into @${file.name}.`,
      '',
      `Component title: ${payload.title}`,
      `Drop target: current preview canvas${slideState ? `, slide ${slideState.active + 1} / ${slideState.count}` : ''}`,
      '',
      `User intent: ${instruction.trim()}`,
      '',
      'Use the referenced component as a reusable pattern or source material. Adapt it to the current page rather than pasting blindly. Keep the visual language consistent with the current project.',
    ].join('\n'));
  }
    ...
    <div className="viewer html-viewer">
      ...
      <div className="viewer-body" ref={previewBodyRef}>
        {source === null ? (
          ...
          <div
            className="comment-preview-layer"
            onDragOver={(event) => {
              if (event.dataTransfer.types.includes('application/x-open-design-component')) {
                event.preventDefault();
                event.dataTransfer.dropEffect = 'copy';
              }
            }}
            onDrop={(event) => {
              const raw = event.dataTransfer.getData('application/x-open-design-component');
              if (!raw) return;
              event.preventDefault();
              try {
                const payload = JSON.parse(raw) as ComponentDropPayload;
                const note = window.prompt('請說明這個元件要放到目前畫布做什麼用途：', '');
                if (!note?.trim()) return;
                sendComponentDropPrompt(payload, note);
              } catch {
                // Ignore malformed external drags.
              }
            }}
          >
            <div
