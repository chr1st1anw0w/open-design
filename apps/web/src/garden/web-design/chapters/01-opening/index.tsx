...
/**
 * Chapter 01 · Opening 「一則崩盤」
 *
 * 4 幕 × 14 個 step：
 *  Scene A · Crash      (0..3)  Figma 股價崩盤
 *  Scene B · Victims    (4..8)  Anthropic 歷次帶崩列表
 *  Scene C · Reveal     (9..11) Claude Design 揭幕
 *  Scene D · Skill      (12..13) 我把它做成了 Skill
 *
 * 真實資料：LegalZoom -20% / CRCL -20% / CrowdStrike -7% / Figma -7%
 *
 * 設計原則：
 *  - 沒有任何"網頁 chrome"（沒有頂部章節標記 / 底部 footer / 永久 hint）
 *  - 每幕透過 SceneFade 優雅交叉淡入淡出，不會有重疊 bug
 *  - 每個 step 推進只增加 1 個新視覺元素
 */

const VICTIMS = [
  { product: 'Claude Cowork',        company: 'LegalZoom',       ticker: 'LZ',   drop: -20.0, field: '法律服務' },
  { product: 'Claude Code Security', company: 'Circle Internet', ticker: 'CRCL', drop: -20.0, field: '雲安全' },
  { product: 'Claude Mythos',        company: 'CrowdStrike',     ticker: 'CRWD', drop: -7.0,  field: '終端安全' },
  { product: 'Claude Design',        company: 'Figma',           ticker: 'FIG',  drop: -7.0,  field: '設計協作', current: true },
];
        ...
        <div className="opening__sceneA">
          ...
          {at(1) && (
            <Reveal kind="rise" duration={620} className="opening__crash-intro">
              ...
              <span className="opening__crash-intro-text">就在前兩天 ——</span>
            </Reveal>
          )}

          {/* 折線圖 —— step 1 開始繪製歷史 / step 2 崩盤 */}
          ...
          {at(2) && (
            <Reveal kind="rise" duration={780} delay={620} className="opening__crash-headline">
              ...
              <em className="opening__crash-emph">的股價崩了。</em>
            </Reveal>
          )}
          ...
          {at(3) && (
            <Reveal kind="rise" duration={520} className="opening__crash-stat">
              ...
              <div className="opening__crash-stat-label">單日跌幅 · 收盤</div>
            </Reveal>
          )}
        </div>
      </SceneFade>
    ...
    >
      ...
      {showEnd && (
        <g className="opening__chart-end">
          ...
          <text x="1740" y="245" textAnchor="end">前日收盤 51.95</text>
          <text x="1740" y="495" textAnchor="end">今日收盤 48.32</text>
        </g>
      )}
    </svg>
  );
    ...
    <div className="opening__skill-card">
      ...
      <div className="opening__skill-card-body">
        <div className="opening__skill-card-row">
          <span className="opening__skill-card-k">支援環境</span>
          <span className="opening__skill-card-v">Cursor · Claude Code · Codex</span>
        </div>
        <div className="opening__skill-card-row">
          <span className="opening__skill-card-k">體量</span>
          <span className="opening__skill-card-v">≈ 400 行 · 含模板庫 ≈ 520 行</span>
        </div>
        <div className="opening__skill-card-row">
          <span className="opening__skill-card-k">來源</span>
          <span className="opening__skill-card-v">Claude Design 系統提示詞 · 提煉 + 改良</span>
        </div>
        <div className="opening__skill-card-row">
