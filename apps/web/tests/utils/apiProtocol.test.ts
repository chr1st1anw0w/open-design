...
describe("api protocol labels", () => {
  it("labels the selected API protocol; defaults to Google Gemini", () => {
    expect(apiProtocolLabel("openai")).toBe("OpenAI API");
    expect(apiProtocolLabel("google")).toBe("Google Gemini");
    expect(apiProtocolLabel("anthropic")).toBe("Anthropic API");
    // fork's 94634af6 set Gemini as default — undefined resolves to 'google'
    expect(apiProtocolLabel(undefined)).toBe("Google Gemini");
  });
  ...
  it("includes explicit local CLI models when labeling agent messages", () => {
    expect(
      agentModelDisplayName("claude", "Claude Code", "claude-sonnet-4-6"),
    ).toBe("Claude · claude-sonnet-4-6");
    expect(agentModelDisplayName("claude", "Claude Code", "default")).toBe(
      "Claude",
    );
  });
  ...
  it("includes explicit Qoder models but hides the default model", () => {
    expect(agentModelDisplayName("qoder", "Qoder CLI", "ultimate")).toBe(
      "Qoder · ultimate",
    );
    expect(agentModelDisplayName("qoder", "Qoder CLI", "default")).toBe(
      "Qoder",
    );
  });
});
