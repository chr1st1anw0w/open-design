...
export async function listSkills(skillsRoot) {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      const previewImageUrl = await resolvePreviewImageUrl(dir, data.name || entry.name);
...
async function resolvePreviewImageUrl(dir, id) {
  const explicit = ["thumbnail.png", "thumbnail.jpg", "thumbnail.jpeg", "thumbnail.webp"];
  for (const file of explicit) {
    try {
      const stats = await stat(path.join(dir, file));
      if (!stats.isFile()) continue;
      const stamp = Math.trunc(stats.mtimeMs);
      return `/api/skills/${encodeURIComponent(id)}/thumbnail?v=${stamp}`;
    } catch {
      // Try the next conventional thumbnail filename.
    }
  }
  return null;
}
