...
export async function writeProjectFile(
  ...
  if (overwrite) {
    await snapshotProjectFile(projectsRoot, projectId, safeName).catch(() => {});
  }
  if (!overwrite) {
    try {
      await stat(target);
...
export async function snapshotProjectFile(projectsRoot, projectId, name) {
  const dir = projectDir(projectsRoot, projectId);
  const safeName = sanitizePath(name);
  const sourcePath = resolveSafe(dir, safeName);
  try {
    await stat(sourcePath);
  } catch {
    return;
  }
  const snapshotDir = path.join(dir, '.snapshots', safeName);
  await mkdir(snapshotDir, { recursive: true });
  const ext = path.extname(safeName) || '';
  const snapshotPath = path.join(snapshotDir, `${Date.now()}${ext}`);
  await writeFile(snapshotPath, await readFile(sourcePath));
  await pruneSnapshots(snapshotDir, MAX_SNAPSHOTS_PER_FILE);
}

export async function pruneSnapshots(snapshotDir, maxCount) {
  let entries = [];
  try {
  ...
  const excess = sorted.slice(0, Math.max(0, sorted.length - maxCount));
  await Promise.all(excess.map((entry) => unlink(path.join(snapshotDir, entry)).catch(() => {})));
}

export async function listSnapshots(projectsRoot, projectId, name) {
  const dir = projectDir(projectsRoot, projectId);
  const safeName = sanitizePath(name);
  const snapshotDir = path.join(dir, '.snapshots', safeName);
  let entries = [];
...
export async function readSnapshot(projectsRoot, projectId, name, timestamp) {
  const dir = projectDir(projectsRoot, projectId);
  const safeName = sanitizePath(name);
  const ext = path.extname(safeName) || '';
  const snapshotPath = path.join(dir, '.snapshots', safeName, `${timestamp}${ext}`);
  return readFile(snapshotPath);
}

export async function restoreSnapshot(projectsRoot, projectId, name, timestamp) {
  const dir = projectDir(projectsRoot, projectId);
  const safeName = sanitizePath(name);
  const ext = path.extname(safeName) || '';
  const snapshotPath = path.join(dir, '.snapshots', safeName, `${timestamp}${ext}`);
  await stat(snapshotPath);
  await snapshotProjectFile(projectsRoot, projectId, safeName);
  const targetPath = resolveSafe(dir, safeName);
  await mkdir(path.dirname(targetPath), { recursive: true });
  await writeFile(targetPath, await readFile(snapshotPath));
}
